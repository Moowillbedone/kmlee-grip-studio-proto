...
- **가격·재고 반영(2026-07-10):** `buildCombos`가 조합 가격/재고를 **대표 옵션(옵션1=첫 축) 값 기준**으로 파생 → 일괄수정에서 옵션1 값 가격/재고 편집 시 조합표에 반영(옵션 1개면 값=조합 1:1 정확). ⚠️ **조합별 개별 가격 실데이터 연동은 step5**(옵션 상품 생성 스펙 확정 후) — 옵션2·3 값 가격은 팝업엔 저장되나 조합표 가격엔 미반영(대표=옵션1 기준).
- **옵션 종류 추가/삭제(2026-07-10):** 팝업 하단 **`+ 옵션 추가` 점선 버튼**(축 추가, `data-addaxis`, `MAX_AXES=3` — 3개면 숨김, `tplAxis(ai)`로 옵션N 템플릿 추가). 각 종류 아래 **`옵션 삭제`**(축 삭제, `data-axdel`, 2개↑일 때). **값 행 추가/삭제(`data-add`/`data-del-ax`)와는 별개** — 전자는 옵션 축, 후자는 옵션 값.
