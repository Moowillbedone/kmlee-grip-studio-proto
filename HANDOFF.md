...
- **⭐ VOC 반영 대개편(2026-07-14, apply36~38 — 최신, 이전 중첩모달 구조 대체):** GRIPPGM-3336 지라 첨부(운영 att_00 + VOC)를 보니 실제 운영은 **상품 옵션 관리 모달 하나**에서 다 처리(별도 [옵션 일괄 수정] 중첩모달 없음)이고, VOC 3요청=①가격·재고 일괄수정 ②옵션 리스트 +아이콘 옵션추가 ③옵션명 클릭 인라인 수정. 우리 v1은 중첩모달(1depth)이 생겨 어긋남 → **모달 하나로 전면 재구성.** 현재 버전 백업: git tag `option-v1-nested-modal`(@4fc89f0), `_archive/index_option_v1_nested-modal.html`, 스크래치패드 동명. **새 구조(renderGroup 전면 교체):** ①`.opt-edit` 인라인 편집 영역(항상 노출, `nameEditOpen`) — 옵션 종류명(`.opt-etype`)·값(`.opt-eval`) 클릭 인라인 수정 + `.opt-evadd`(+옵션 추가=값1개)·`.opt-evdel`(값삭제)·`.opt-eaxadd`(+옵션 종류 추가 최대3)·`.opt-eaxdel`(종류삭제) ②일괄 수정 바 `.opt-bulk`(선택 N개 + 가격/재고 입력 + `.opt-bulk-apply` 선택 적용) ③조합 표: 행 선택 체크박스 `.opt-rowsel`/전체선택 `.opt-selall`(선택은 `c._sel` 트랜지언트, **syncSel로 re-render 없이 부분갱신** — 일괄 입력값·포커스 보존), 가격/재고/자체옵션코드(`.opt-code-inp`) 전부 인라인, 판매여부 토글. 종류/값명 편집은 `syncComboLabels`로 헤더·라벨만 부분 갱신(포커스 보존). **[옵션 일괄 수정] 버튼 → [옵션명 수정하기]로 라벨 변경**(#optEditNameBtn), 클릭 시 `nameEditOpen` 토글(.opt-edit 접기/펼치기, active 클래스). 구 중첩모달(#optNameModal/#optDelAllModal)·renderBody/openName 등은 **미사용 dead code로 잔존**(제거 안 함). 데모 6스텝 새 플로우로 교체(인라인 편집→가격/재고/코드→체크박스 일괄→선택 적용, 커밋 없이 스팟라이트). 인앱 PRD(개요·플로우·명세1~3) 새 구조 반영. **검증:** 2개 선택→일괄 10,000/100 적용(입력값 유지)·옵션값→레드/종류→색상 동기화·+값추가 4조합·+종류추가 2축·코드 A-1·재오픈 저장유지·데모 전스텝·콘솔0. **UI 정리(2026-07-14, apply39~41, "정신없다" 피드백):** ①일괄 수정 = 상시 노출 바 제거 → **`.opt-info-head`에 [일괄 수정] 버튼(#optBulkBtn, 선택 시 활성·"일괄 수정 (N)")** → 클릭 시 **작은 팝업 #optBulkModal**(가격/재고 입력 + 적용, 선택 조합에만, 입력 항목만 반영). 체크박스는 유지(3338 가격수정·3357 선택삭제 패턴과 동일). ②**옵션 종류·값 편집 영역(.opt-edit) 기본 접힘**(nameEditOpen 기본 false, openModal마다 false 리셋) → **[옵션명 수정하기]로 펼침/접기**(운영 att_00처럼 기본은 요약+버튼+표만). updateBulkBtn()이 renderGroup·syncSel 양쪽에서 버튼 상태 갱신(적용 후 선택 해제→비활성). 데모 6스텝 재정렬(진입→가격/재고/코드 인라인→[옵션명 수정하기] 펼침→체크박스 선택→[일괄 수정] 팝업). 인앱 PRD도 반영 필요분 반영. 검증: 기본 접힘·버튼 비활성, 2선택→"일괄 수정(2)"→팝업 12000/88 적용(3,100/88)·적용 후 비활성, 토글 열기/닫기, 데모 팝업까지, 콘솔0. **VOC 재정렬(2026-07-14, apply42~44, 지라 첨부 3장 반영):** ①**옵션명·옵션값 표 인라인**: 조합표 헤더=옵션명(`.opt-hname` 인풋, 축 type)·본문=옵션값(`.opt-vname` 인풋, 값명) 둘 다 클릭 인라인 수정(가격·재고처럼). `valIdxOf`로 조합별 값 인덱스 매핑, 다축이면 같은 값 인풋 형제 동기화. ②**[옵션명 수정하기] → 팝업(#optNameModal 재활용, 첨부2 ASIS)**: 이름만 편집(옵션 종류/값 2컬럼 `.onm-*`, +값추가/+종류추가/삭제, 취소/수정하기). renderBody/readBody 이름 전용으로 단순화, editNameBtn→openName 재바인딩(구 인라인 토글 폐기). **(2026-07-14 재수정, apply45) 팝업은 저장된 옵션 종류·값 이름 "수정만"** — 값/종류 추가·삭제 버튼 전면 제거(첨부 ASIS 일치). renderBody가 onm-typei/onm-val 인풋만 렌더(onm-x/onm-vadd/onm-axadd/onm-axdel dead CSS). 옵션 추가/삭제는 이 팝업에 없음(VOC②는 PRD TO-BE로만).
- **가격 검증(2026-07-14, apply46):** ①**옵션 상품 라이브가**(리스트 행 `.pi-opt-live-input`)에 3338 인라인 패턴 적용 — 입력 중 프리뷰(부호 차단), **Enter/blur 확정 시 200원 미만이면 미적용·직전 유효값(data-prev)으로 되돌림 + 실패 토스트**, 200원 이상이면 fmt+성공 토스트(`commitOptLive`, delegated keydown/focusout, optLiveLock 60ms 중복방지, OPT_LIVE_MIN=200). ②**상품 옵션 관리 옵션 정보 표의 가격/재고(`.opt-price-inp`/`.opt-stock-inp`)는 자유 입력**(200원 제한 없음) — 입력 시 `[^\d,]` 제거로 **부호(마이너스 등)만 차단**. 가격이 라이브가보다 낮으면 addPrice가 음수로 파생되나 표시값은 입력값 그대로(자유). 검증: 라이브가 150→8,900 되돌림·5000/7000 적용·"-300"→300, 표 가격 50 허용·"-90"→90·재고 "-5"→5, 콘솔0.
- **PRD 명세 리스트/팝업 동작 구분 정리(2026-07-14, apply47, 사용자 "혼동" 피드백):** 인앱 PRD가 리스트 동작과 [옵션] 팝업 동작을 섞어 혼동 → **명세를 위치별로 재구성**: 명세1=[리스트→팝업] 진입&요약, 명세2=[팝업] 옵션 정보 표·인라인 수정, 명세3=[팝업] 옵션명 수정·일괄 수정, 명세4=[리스트] 가격 모델 & 리스트 반영. 각 title에 [리스트]/[팝업] 라벨, overview 해결란·flowHTML도 "리스트(홈) vs 팝업" 2단으로. **명세4 ASIS 교체** = "옵션 상품의 경우 라이브가 노출이 안되어 가격이 명확하지 않음"(사용자 지정 문구). **컨플루언스 2337406981 v6 동기화**(apply_prd_option2.py): 히스토리 ver0.2, AS-IS/TO-BE(가격표기 ASIS 정정), 기능요구 4항목(리스트/팝업), 검증규칙 V-1~V-7 현재 동작 기준 재작성(구 V-8/9·전체삭제·초기화 제거), 화면UX·영향범위·플로우 텍스트 최신화 + **플로우차트 재생성**(draw_option.py: 리스트 vs 팝업 구분, option_flow.png 교체). "Preview unavailable" 없이 img 렌더 확인. **(2026-07-14 후속) AS-IS/TO-BE 명확화**: 모호한 "수정 반영" 행 제거, 4행으로 정리(옵션 가격·재고 수정 / 옵션명 수정 / 옵션 상품 가격 표기[리스트] / **조합별 재고 지표[판매 가능·주문·점유] 추가**=비즈센터 연동 읽기전용). 인앱 명세2에도 지표 asis/tobe 추가. 컨플루언스 **v7**.
- **[옵션명 수정하기] → [옵션 정보 수정] 복원(2026-07-14, apply48~49, 사용자 요청):** 지난 apply45의 "이름 수정 전용" 결정을 되돌림. 버튼·팝업명 **[옵션 정보 수정]**(#optEditNameBtn), renderBody를 **옵션 종류·값 + 추가/삭제 + 이름 수정** 버전으로 복원(.onm-x 값삭제/.onm-vadd 값추가/.onm-axadd 종류추가/.onm-axdel 종류삭제 — 이 클릭 핸들러들은 apply45 이후에도 dead로 남아있어 renderBody만 되살리면 동작). 노트 "옵션 종류·값 추가·삭제/이름 수정(종류 최대 3·값 무제한)". 인앱 PRD(overview·flow·명세3·데모스텝)·컨플루언스 **v8**(히스토리 ver0.3·기능요구No3·V-6·화면UX) 동기화. 검증: 값 추가(라지→4조합)·종류 추가(사이즈)·값 삭제·수정하기 반영·데모 "옵션 정보 수정 팝업"·콘솔0. (태그형 Artifact 데모는 폐기.)
- **[옵션 정보 수정] 팝업 = 어제 전체 버전으로 재복원(2026-07-14, apply50, 사용자 "기록 시점 UI와 다르다"):** apply48/49의 축소판(옵션명/값+추가/삭제만)이 사용자가 기록해두라던 시점(VOC 리워크 직전=option-v1-nested-modal 태그) 화면과 달라서, **팝업만** 그 전체 버전으로 되돌림 — 모달 900px(opt-bulk-modal), **섹션1 옵션명/값**(.optb-axis2: 옵션명 input + .optb-vrow2 값 input+삭제 + 옵션값 추가 + +옵션 추가) + **섹션2 옵션 상세**(.optd: 조합별 추가가격/재고/판매여부/자체옵션코드 표 + [일괄 입력]), 하단 [초기화]/[옵션 적용 하기]. renderBody/readBody를 apply43 OLD_RB(전체)로, 모달 HTML을 OLD_MODAL 구조로(이름만 [옵션 정보 수정] 유지), nameReset→draft 초기화. 관련 CSS(.optb-/.optd-/.opt-bulk-modal)·클릭핸들러(data-addaxis/axdel/addval/del-ax/dsale/#optdBulkBtn/#optdBulkApply)는 계속 잔존했어서 renderBody만 되살리면 동작. 검증: 조합 추가가격 500·재고 77·코드 A1 + 옵션값 추가(4조합) → 메인 표 9,400/77 반영·콘솔0. 직전(축소판) 백업: git tag option-정보수정-v2-add-delete + scratchpad/index_optinfo_v2_before_restore.html. ⚠️ 인앱 PRD 명세3·데모·컨플루언스는 아직 축소판(추가·삭제·이름만) 서술 — "화면으로만" 요청이라 미변경, 필요 시 동기화.
- **옵션 정보 수정 섹션1 UI 개선(2026-07-14, apply51, 첨부3 형태):** 섹션1(옵션명/값)만 정리 — 스택형(옵션명 N/옵션값 N/큰 '옵션값 추가' 버튼)에서 **2컬럼(옵션 종류｜옵션 값)**으로. 헤더 1회(.oe-head), 축마다 .oe-axis(좌 .oe-typecol=종류 input+옵션 삭제[축>1일 때], 우 .oe-valcol=값 행들). **값 행 인라인 버튼**: 첫 행 버튼 없음 / 중간 행 [삭제] / 마지막 행 [추가]+[삭제](.oe-vadd/.oe-vdel, data-addval/data-del-ax·del-vi). 큰 'optb-addval' 버튼 제거. **[+ 옵션 추가](종류, optb-addaxis)는 섹션1 하단 단일**. 클래스 이중부여(optb-axis2 oe-axis / optb-vrow2 oe-vrow / optb-name / opt-type-input)로 readBody·핸들러 그대로 동작. **섹션2(옵션 상세 .optd)는 손대지 않음**(사용자 지시). 검증: 추가→4행(추가버튼 마지막행 이동)·삭제→3행·종류 추가 2축(옵션 삭제 노출)·콘솔0. **(apply52) 옵션 상세 헤더의 [일괄 입력] 버튼(#optdBulkBtn) 제거**(사용자 요청). bulkBar/핸들러는 dead로 잔존(무해).
- **(apply53) 새 조합 기본값 공백화(사용자 요청):** 옵션/옵션값 추가로 생기는 새 조합은 추가가격·재고가 자동 채워지지 않고 **빈값**. `emptyCombo(){addPrice:'',stock:'',sale:true,code:''}` 추가, **resizeCombos가 신규 조합에 emptyCombo() 사용**(구 defaultCombo 100·10 시퀀스 대신). **buildCombos(초기 상품 조합)는 defaultCombo 유지**(기존 데모 데이터 보존). → 옵션 상세에서 새 조합 행의 추가가격/재고 인풋 공백(placeholder), 기존 조합은 값 유지. 메인 표는 빈 addPrice→가격=라이브가, 빈 stock→재고 0으로 파생. 검증: 값 '라지' 추가 → 4번째 조합 추가가격/재고 공백·기존 3개 100/200/300·10/20/30 유지·콘솔0. ③**컬럼 추가+재배치(req5)**: `[선택] 옵션명·옵션값·재고·판매 가능·주문·점유·가격·판매 여부·자체 옵션 코드`. **판매 가능/주문/점유 = 읽기전용**(occOf/ordOf 목값, 판매가능=재고−점유; 재고 수정 시 판매가능 실시간 갱신). 라벨은 '수량' 뗌. 모달 폭 720→1000px. **★비즈센터 연동 메모(PRD/로직용, 화면표기 X):** 점유 수량·판매 가능 수량·주문 수량은 비즈센터에서 연동되는 값 — 스튜디오에서 편집 아님(읽기전용). 데모 6스텝 재정렬(진입→표 인라인→옵션명 팝업→체크박스 선택→일괄 팝업, 각 팝업 상태 검증). 검증: 표 옵션명/값 인라인·팝업 이름수정 반영·팝업 값추가(이름 채우면 조합 확장)·판매가능 재고연동·데모 모달상태·콘솔0.
- **★(apply54~57) [옵션 정보 수정] 팝업 조합 정합성 치명버그 수정(2026-07-14, 사용자 "치명적 오류 정상화"):** 증상=옵션 축/값을 추가·편집하면 데이터가 옵션1로 쏠려 매칭 안 됨, 삭제 시 데이터 밀림. **두 근본 원인:** ①이름/값 편집 blur의 `change` 핸들러가 `renderBody()`로 섹션 전체 `innerHTML` 재생성 → 포커스 유실 + **blur→재생성 순간 방금 누른 [추가]/[삭제]/[+옵션추가] 버튼이 detach돼 click이 삼켜짐**(그래서 "옵션2에 값이 안 붙고 옵션1 연장선처럼 보임"). ②조합 데이터를 **평면 인덱스**로만 보존(`resizeCombos`) → 구조 변경 시 각 조합의 (축별 값 좌표)가 바뀌는데 옛 가격·재고가 엉뚱한 조합에 붙음(가격 뒤섞임)·삭제 시 밀림. **수정:** (apply54) `change`→`readBody()+syncDetailLabels()`(조합표 헤더/라벨 textContent만 부분갱신, 재생성·detach 없음→클릭 삼킴 해소·포커스 유지) + `remapCombos`(좌표 기준 보존) 도입. (apply55) 값 삭제에 `valMap`(newValIdx→oldValIdx, dv 이후 +1) → 중간 값 삭제 시 살아남는 값이 자기 데이터 유지(밀림 없음). (apply56) [옵션 적용 하기]도 valMap. **(apply57 — 적대적 검증 워크플로우가 잡은 2차 결함 반영) `remapCombos(oldAxes,newAxes,oldCombos,axisMap,valMap)`로 일반화**(구 `sameDim` 게이트 제거): `axisMap[na]`=대응 old축(-1=신규축), `oldToNew`로 old축 순회하며 좌표 역산. **축 추가**(새 축은 맨끝·값1개→leading 좌표 불변)·**값1개 축 삭제**(중간 포함)는 데이터 **보존**, **값 여러개 축을 통째 제거**하는 모호한 좌표만 공백. 핸들러: 축추가/값추가=`syncCombos`(identity), 축삭제/1값축 제거=shift axisMap, 값삭제=`(…,null,vmap)`, 적용=`(…,cleanAxisMap,cleanValMaps)`. 검증: 라이브 9+시나리오(축추가 100/200/300 보존·값추가 새조합만 공백·중간값삭제 밀림0·중간축(1값)삭제 정확대응·2축 색상중간값삭제·이름편집 안샘+포커스유지·클릭삼킴0·적용커밋·콘솔0) 전부 통과 + 4-관점 적대적 워크플로우(radix·event-order 무결, 데이터손실 3건은 apply57에서 해소돼 verify 전건 REFUTED). 백업 .bak54~57. ⚠️ **컨플루언스 PRD 2337406981 + 인앱 PRD/데모는 아직 구버전 서술 — 사용자 확인 후 재동기화 예정.**
