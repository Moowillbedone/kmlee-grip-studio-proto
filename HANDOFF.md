...
- **옵션명 수정 중첩 팝업 `#optNameModal`**(z-index 120, `#optNameBody`): **옵션 종류별 블록**(종류 input + 값 input들) → `[수정하기]` → `state.axes[].type/values[]` 갱신 → `renderGroup()`로 표·필터칩(종류별 줄)·요약("옵션1(3) × 옵션2(3) = 총 9개 조합") 즉시 반영. (옵션 종류 최대 3개 지정은 별도 화면 — 여기선 데모 데이터.)
- 필터 체크박스는 축별(`data-all`=축idx, `data-ax`/`data-vi`) — 4단계(일괄 대상 선택)용 기반. **PRD/데모/컨플루언스 미등록**(4단계까지 끝난 뒤 요청 시).
