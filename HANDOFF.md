...
- **삭제:** 상품 행 🗑(`.pr-del`) → 확인 모달(`#deleteModal`, "비즈센터 상품은 유지") → 제거 + 토스트. UP 목록/프리뷰 자동 반영. 비즈센터 미반영.
...
- 페이지: 가격 **2306113587**(3338) / 삭제 **2319581187**(3357) / 채팅 **2319515671**(3224).
...
**구현됨(최종 정책 — 본인만 보기):** 답변 인라인 박스(`#chatReply`)에 **"본인만 보기" 토글(`#crLiveBox`, 기본 OFF)**. `registerQA(m,a,priv)`에 `priv` 플래그. **OFF(기본)=전체 공개**(모든 유저 Q&A + 방송 프리뷰 띠지 노출), **ON=본인만**(문의한 본인에게만, 다른 유저·방송 미노출). Q&A 탭 항목 배지/토글 `.qa-live`(🌐 전체 공개 / 🔒 본인만·닉)로 사후 전환(data-priv). `updateOvQA()`는 **priv=false(전체 공개)인 최신 항목만** 띠지 표시. PRD 기능4 "qavis"(GRIPPGM-3402, 이름 "…Q&A 노출 범위(전체/본인만)") + 데모(시작→답변작성→노출범위 토글→전체공개 기본→본인만). 검증: 기본→띠지 ON·"🌐 전체 공개", 본인만→띠지 X·"🔒 본인만·닉", Q&A 토글로 전환 시 띠지 반영. (초기엔 "라이브 노출 on/off"였다가 사용자 요청으로 "전체 공개 기본 / 본인만 토글"로 정책 변경함.) 컨플루언스 PRD는 요청 시(부모 2306867212, PIL 플로우차트).
