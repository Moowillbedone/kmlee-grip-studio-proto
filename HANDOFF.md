...
- **옵션명 수정 중첩 팝업 `#optNameModal`**(z-index 120): 옵션 종류 1 + 옵션 값 3 input → `[수정하기]` → `state.type`/`state.values[].name` 갱신 → `renderGroup()`로 표·필터칩·요약 즉시 반영. (옵션 종류는 최대 3개까지 가능하나 지정은 별도 화면 — 여기선 1종류·3값 케이스만.)
- 상태: `state={type, values:[{name,price,stock,sale}]}`. 체크박스는 4단계(일괄 대상 선택)용 기반. **PRD/데모/컨플루언스 미등록**(4단계까지 끝난 뒤 요청 시).
