...
- **옵션 종류 추가/삭제(2026-07-10):** 팝업 하단 **`+ 옵션 추가` 점선 버튼**(축 추가, `data-addaxis`, `MAX_AXES=3` — 3개면 숨김, `tplAxis(ai)`로 옵션N 템플릿 추가). 각 종류 아래 **`옵션 삭제`**(축 삭제, `data-axdel`, **항상 노출**). **값 행 추가/삭제(`data-add`/`data-del-ax`)와는 별개** — 전자는 옵션 축, 후자는 옵션 값.
